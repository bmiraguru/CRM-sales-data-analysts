...
SELECT 
    regional_office, AVG(close_value) as avgclosevalue
FROM
    (SELECT 
...
FROM 
    sales_pipeline;
	   ...
	   select regional_office, count(*) as total_top_performers from (select * from (select *,ntile(3) over(order by avgdaysofengagement asc) as KPI from (select b.sales_agent,avgdaysofengagement,regional_office,manager from (select sales_agent, avg(totaldaysofengagement) as avgdaysofengagement from( sELECT 
			sp.sales_agent,
				sp.engage_date,
				sp.close_date,
				DATEDIFF(close_date, engage_date) AS totaldaysofengagement
...
select regional_office, count(*) as total_top_performers  from (select * from (select *,ntile(3) over(order by close_value desc) as KPI from (select sp.sales_agent,close_value,st.regional_office from sales_pipeline sp join sales_teams st on st.sales_agent=sp.sales_agent)a) b where KPI=1)c group by 1 order by 2 desc;
...
cte2 as (select count(*)  as totaldeals from sales_pipeline)
